...
class CustomJoint(Joint):
  ...
  def _parse(self, xml, socket_parent_frame, socket_child_frame, pointer):
    ...
    for idx in order:
      ...
      # Handle a "SimmSpline" or "NaturalCubicSpline" transformation with a quartic approximation
      ...
      elif t.find("PolynomialFunction") is not None:

        # Handle PolynomialFunction - convert to quartic constraint like SimmSpline
        coefficients = str2vec(t.find("PolynomialFunction/coefficients").text)
        ...
        # Polynomial coefficients are from highest to lowest order
        # We need to convert to MuJoCo polycoef format: [a0, a1, a2, a3, a4] where y = a0 + a1*x + a2*x^2 + ...
        # OpenSim format: [an, an-1, ..., a1, a0] (highest to lowest)
        poly_order = len(coefficients) - 1
        
        # Reverse to get lowest to highest order
        coefficients_reversed = coefficients[::-1]
        
        # Pad to 5 coefficients for MuJoCo quartic
        polycoef = np.zeros(5)
        for i, c in enumerate(coefficients_reversed[:5]):
          polycoef[i] = c
        
        # Generate sample points to estimate range
        if "range" in params:
          x_min, x_max = params["range"]
        else:
          x_min, x_max = -2.0, 2.0  # Default range in radians
        
        x_values = np.linspace(x_min, x_max, 100)
        y_values = np.polyval(coefficients, x_values)
        joint_range = np.array([np.min(y_values), np.max(y_values)])
        
        # Create fit function for updating values
        fit = lambda x: np.polyval(coefficients, x)
        ...
        # Update default value (if it is set)
        if "user" in params:
          params["user"] = fit(params["user"])
        
        # Update _transform_value
        if "_transform_value" in params:
          params["_transform_value"] = fit(params["_transform_value"])
        
        # Add a joint constraint between this joint and the independent joint
        etree.SubElement(cfg.M_EQUALITY, "joint", joint1=params["name"], joint2=independent_coordinate,
                         polycoef=vec2str(polycoef),
                         solimp="0.9999 0.9999 0.001 0.5 2", active="true")
        
        # Update motion type to dependent
        params["_motion_type"] = "dependent"
  ...
  def _designate_dof(self, t, params, coordinate):
    ...
    if t.find("SimmSpline") is not None:
      ...
      # Handle PolynomialFunction - treat as non-designated if non-linear
      coefficients = str2vec(t.find("PolynomialFunction/coefficients").text)
      
      # Check if it's essentially linear (only last two coefficients are non-zero)
      # Polynomial coefficients are from highest to lowest order
      if len(coefficients) == 2:
        # Linear: ax + b
        polycoef = np.array([coefficients[1], coefficients[0], 0, 0, 0])
      elif len(coefficients) > 2 and np.allclose(coefficients[:-2], 0, atol=1e-8):
        # Higher order but effectively linear
        polycoef = np.array([coefficients[-1], coefficients[-2], 0, 0, 0])
      else:
        # Non-linear polynomial - cannot designate as DoF
        logger.info(f"PolynomialFunction in {params['name']} is non-linear, not designating as DoF")
        return False

    else:
      logger.critical("Only LinearFunction, SimmSpline, and PolynomialFunction transformation types are currently implemented")
      raise NotImplementedError
